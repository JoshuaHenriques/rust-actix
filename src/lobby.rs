...
impl Handler<Disconnect> for Lobby {
    ...
    // Send message to everyone in the lobby that that user_id has disconnected
...
impl Handler<ClientActorMessage> for Lobby {
    ...
    fn handle(&mut self, msg: ClientActorMessage, _: &mut Context<Self>) {
        // if message is \w then we are whispering to a specific id
        if msg.msg.starts_with("\\w") {
            if let Some(id_to) = msg.msg.split(' ').collect::<Vec<&str>>().get(1) {
                // not safe, in the future to check if its a valid uuid
            ...
            self.rooms.get(&msg.room_id)
                ...
                .iter()
                .filter(|conn_id| *conn_id.to_owned() != msg.id)
                .for_each(|client| self.send_message(&format!("{}: {}", msg.id, &msg.msg), client));
        }
    }
}